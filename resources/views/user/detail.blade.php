        ...
        <div class="card mt-3">
            <div class="card-body">
                <h5 class="card-title d-flex justify-content-between">
                    <span class="text-primary fw-bold">Chỉnh sửa tài khoản</span>
                    ...
                    <div class="row my-3">
                        <div class="col-4">
                            ...
                            <select name="role" class="form-select {{ $errors->has('role') ? 'is-invalid' : '' }}">
                                ...
                                @foreach ($roles as $role)
                                    <option
                                        value="{{ $role->name }}" {{ old('role', $user['role']) == $role->name ? 'selected' : '' }}>
                                        {{ $role->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-4">
                            ...
                            <input type="text" class="form-control" disabled
                                   value="{{ $user['created_at']->format('H:i d/m/Y') }}">
                        </div>
                    </div>
                                                ...
                                                @foreach($itemGroupChild['value'] as $key => $item)
                                                    <div class="form-check form-check-inline me-5">
                                                        <input name="permissions[]" class="form-check-input"
                                                               ...
                                                               value="{{$item['value']}}"
                                                        {{ in_array($item['value'], old('permissions', $user['permissions']->toArray())) ? 'checked' : '' }}
                                                        >
